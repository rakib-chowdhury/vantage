...
{{--<div class="panel panel-success" style="background-color: #329c3b; color: whitesmoke; padding: 7px;"><span--}}
    {{--class="fa fa-smile-o">Successfully send...</span>--}}
    {{--</div>--}}
{{--@endif--}}
            ...
            <div class=" panel col-md-12" style="border-color: black; margin-right: 2px; margin-bottom: 0px;">
                <div class="row">
                    <div class="col-md-12" style="padding: 0px">
                        <div class="col-md-1">
                            <a href="{{url('my_account/profile')}}">
                                <img style="width: 50px; height: 50px; border-radius: 50%"
                                     src="{{url('public/images/emp')}}/{{$account_info[0]->employee->image}}"
                                     alt="">
                            </a>
                        </div>
                        <div style="padding-top:10px;" class="col-md-8">
                            <a href="{{url('my_account/profile')}}">
                                {{$account_info[0]->employee->name}}
                            </a>
                        </div>
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form onsubmit="return checkPass()"
                                                  action="{{url('doctor/update_password')}}" method="post">
                                                {{csrf_field()}}
                                                <div class="row">
                        ...
                        <div class="col-md-1" style="text-align: right; padding-top: 10px;">
                            <a style="color: red" href="{{url('logout')}}"><i><span>Logout</span></i></a>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <td>
                                ...
                                <div id="newMSG" class="modal fade about-us-modal" data-keyboard="false"
                                    ...
                                    <div class="modal-dialog modal-lg" role="document">
                                        <form onsubmit="return chkMSG()" action="{{url('message_post')}}"
                                              method="post"
                                              enctype="multipart/form-data">
                                            {{csrf_field()}}
                                            <input type="hidden" name="sender" value="{{$account_info[0]->emp_id}}">
                                            <div class="modal-content">
                                                ...
                                                <div class="form-group" style="text-align: center">
                                                    ...
                                                    <a href="{{url('my_account/show')}}" class="btn btn-danger">
                                                        Close
                                                    </a>
                                                </div>
                                            </div>
                                ...
                                <a href="{{url('my_account/show')}}" data-placement="top" title="Refresh"
                        ...
                        <tr>
                            <td>
                                ...
                                foreach ($client as $key=>$c){
                                    if($key == 0){?>
                                        <input type="hidden" name="fclient" id="fclient" value="{{$c->id}}">
                                        <input type="hidden" name="fclientname" id="fclientname" value="{{$c->name}}">
                                    <?php }
                                    ?>

                                    <a href="javascript:void(0)"
                                       onclick="getMessage('<?=$c->id?>','<?=$c->name?>')">
                                        <img style="padding-left:5px; width:50px; height: 50px; border-radius: 50% " src="{{url('public/images/emp/'.$c->image)}}"
                                             alt="user-img"
                                             class="img-circle">
                                        <span style="padding-left: 10px;">{{$c->name}}</span>
                                    </a>
                                    <br>
                                    <hr>
            ...
            success: function (result) {
                ...
                if (result.length == 0) {
                    ...
                    $.each(result, function (i, v) {
                        ...
                        trow += '<img style="width: 30px; height: 30px; border-radius: 50% " src="{{url('/public/images/emp/')}}/' + v['sender']['image'] + '" alt="">';
                        trow += '</td>';
